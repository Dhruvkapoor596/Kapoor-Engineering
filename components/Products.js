      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="flex flex-col items-center text-center mb-20">
          <div className="inline-flex items-center space-x-2 mb-6 bg-white/5 border border-white/10 px-4 py-1.5 rounded-full">
            ...
            <span className="text-slate-300 text-[10px] font-bold tracking-[0.2em] uppercase">Core Materials</span>
          </div>
          <h2 className="text-4xl md:text-6xl font-black text-white tracking-tight uppercase mb-6">
            Industrial <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#EAB308] to-yellow-500">Supply</span>
          </h2>
        </div>
            ...
            <div key={product.id} className="group relative bg-[#0a0a0a] rounded-3xl border border-white/10 overflow-hidden hover:border-[#EAB308]/50 hover:bg-white/[0.02] transition-all duration-500 flex flex-col h-full">
              ...
              <div className="p-8 flex-grow flex flex-col justify-between relative z-20 -mt-10">
                <div>
                  ...
                  <p className="text-slate-400 text-sm leading-relaxed mb-8 font-light">{product.description}</p>
                </div>
                
                {product.inStock ? (
                  <Link href="/contact" className="inline-flex items-center text-xs font-bold text-white uppercase tracking-widest group-hover:text-[#EAB308] transition-colors">
                    Request Quote <ArrowRight className="w-4 h-4 ml-3 opacity-0 -translate-x-4 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-300" />
                  </Link>
                ) : (
